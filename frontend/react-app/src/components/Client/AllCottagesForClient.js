...
const AllCottagesForClient = () => {
    ...
    useEffect(() => {
        axios
            .post("http://localhost:8080/cottages/getAllCottagesForClient/"+localStorage.getItem("userEmail"), {
                "beginDate": "",
                "endDate": "",
                "city": "",
                "country": "",
    ...
    const sortList = (field) => {
        ...
        if (field === 'title') {
            ...
            alert(titleAsc);
            axios
                .post("http://localhost:8080/cottages/sort/title/" + ascTitle, rows, {
                    headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
                }
                )
                .then((res) => {
        ...
        if (field === 'city') {
            ...
            axios
                .post("http://localhost:8080/cottages/sort/city/" + ascCity, rows, {
                    headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
                }
                )
                .then((res) => {
        ...
        if (field === 'country') {
            ...
            axios
                .post("http://localhost:8080/cottages/sort/country/" + ascCountry, rows, {
                    headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
                }
                )
                .then((res) => {
        ...
        if (field === 'average_grade') {
            ...
            axios
                .post("http://localhost:8080/cottages/sort/average_grade/" + ascAverageGrade, rows, {
                    headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
                }
                )
                .then((res) => {
        ...
        if (field === 'price') {
            ...
            axios
                .post("http://localhost:8080/cottages/sort/price/" + ascPrice, rows, {
                    headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
                }
                )
                .then((res) => {
        ...
        <TableBody>
            {rows.map((row, index) => (
                <TableRow key={index} >
                    ...
                    <TableCell><Link to={`/client/quickReservations/getForCottage/${row.id}`}><Button>Get Quick Booking Reservations</Button></Link></TableCell>
                    {(submited && beginDateSearch !== "" && endDateSearch !== "") ? <TableCell><Link to={`/client/reservation/cottage/${row.id}/${beginDateSearch}/${endDateSearch}`}><Button>Make reservation</Button></Link></TableCell> : ""}
...
export default AllCottagesForClient;