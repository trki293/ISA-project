...
const GetCottageQuickBookingsForClient = () => {
    ...
    useEffect(() => {
        ...
        axios
            .get("http://localhost:8080/cottage_quick_bookings/getQuickBooking/client/" + localStorage.getItem("userEmail") + "/cottage/" + splitID[6], {
    ...
    const createCottageReservation = (id) => {
        axios
            .post("http://localhost:8080/cottage_quick_bookings/createCottageReservationForQuickBooking/" + id + "/client/" + localStorage.getItem("userEmail"), {}, {
                headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
            }
            )
            .then((res) => {
        ...
        <TableBody>
            {rows.map((row, index) => (
                <TableRow key={index} >
                    ...
                    <TableCell>{row.cottage.id}</TableCell>
                    <TableCell>{row.beginDate}</TableCell>
                    <TableCell>{row.endDate}</TableCell>
                    <TableCell>{row.orginalPrice}</TableCell>
                    <TableCell>{row.priceWithDiscount}</TableCell>
                    <TableCell><Button onClick={() => createCottageReservation(row.id)}>MAKE RESERVATION</Button></TableCell>
                </TableRow>
            ))}
        </TableBody>
    );
...
export default GetCottageQuickBookingsForClient;