...
const GetInstructionsForClientComplaint = () => {
    ...
    useEffect(() => {
        ...
        axios
            .get("http://localhost:8080/instructions_reservations/getInstructorsForClientComplaint/" + localStorage.getItem("userEmail") , {
    ...
    const TableContent = (
        <TableBody>
            {rows.map((row, index) => (
                <TableRow key={index} >
                    <TableCell>{row.instructorId}</TableCell>
                    <TableCell>{row.instructorFirstName}</TableCell>
                    <TableCell>{row.instructorLastName}</TableCell>
                    <TableCell><Link to={`/client/complaint/instructor/${row.instructorId}`}><Button>CHOOSE</Button></Link></TableCell>
                </TableRow>
            ))}
        </TableBody>
    );
...
export default GetInstructionsForClientComplaint;