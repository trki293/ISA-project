...
const AllInstructionsForClient = () => {
    ...
    useEffect(() => {
        axios
            .post("http://localhost:8080/instructions/getAllInstructionsForClient/"+localStorage.getItem("userEmail"), {
                "beginDate": "",
                "endDate": "",
                "city": "",
                "country": "",
    ...
    const sortList = (field) => {
        ...
        if (field === 'title') {
            ...
            alert(titleAsc);
            axios
                .post("http://localhost:8080/instructions/sort/title/" + ascTitle, rows, {
                    headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
                }
                )
                .then((res) => {
        ...
        if (field === 'city') {
            ...
            axios
                .post("http://localhost:8080/instructions/sort/city/" + ascCity, rows, {
                    headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
                }
                )
                .then((res) => {
        ...
        if (field === 'country') {
            ...
            axios
                .post("http://localhost:8080/instructions/sort/country/" + ascCountry, rows, {
                    headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
                }
                )
                .then((res) => {
        ...
        if (field === 'average_grade') {
            ...
            axios
                .post("http://localhost:8080/instructions/sort/average_grade/" + ascAverageGrade, rows, {
                    headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
                }
                )
                .then((res) => {
        ...
        if (field === 'price') {
            ...
            axios
                .post("http://localhost:8080/instructions/sort/price/" + ascPrice, rows, {
                    headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
                }
                )
                .then((res) => {
    ...
    const subscribeOrUnsubscribe = (instructionID) => {
        console.log(instructionID);
        axios
            .put("http://localhost:8080/instructions/subscribe/" + localStorage.getItem("userEmail") + "/instructions/" + instructionID, {},  {
                headers: { Authorization: `Bearer ` + localStorage.getItem('token'), consumes: 'application/json' }
            }
            )
            .then((res) => {
                window.location.href="http://localhost:3000/client/instructionsForClient";
            });
    }
        ...
        <TableBody>
            {rows.map((row, index) => (
                <TableRow key={index} >
                    ...
                    <TableCell>{row.maxNumberOfPeoples}</TableCell>
                    <TableCell>{row.averageGrade}</TableCell>
                    <TableCell>{row.pricePerHour}</TableCell>
                    <TableCell><Link to={`/client/quickReservations/getForInstructions/${row.id}`}><Button>Get Quick Booking Reservations</Button></Link></TableCell>
                    {(submited && beginDateSearch !== "" && endDateSearch !== "") ? <TableCell><Link to={`/client/reservation/instructions/${row.id}/${beginDateSearch}/${endDateSearch}`}><Button>Make reservation</Button></Link></TableCell> : ""}
                    {(row.userSubscribed) && <TableCell><Button onClick={()=>subscribeOrUnsubscribe(row.id)}>Unsubscribe</Button></TableCell>}
                    {(!row.userSubscribed) && <TableCell><Button onClick={()=>subscribeOrUnsubscribe(row.id)}>Subscribe</Button></TableCell>}  
                </TableRow>
            ))}
        </TableBody>
    );
...
export default AllInstructionsForClient;